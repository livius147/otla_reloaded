...
int lee_cpc_bin(const char *filename, ts_sbb_header * head_sbb,
		ts_sbb_block * blocks_sbb)
{
	...
	if (!x) {
		...
		strncpy(head_sbb->nombre, (char*)(header_amsdos->filename), 16);
		strcpy(blocks_sbb[0].blockname, (char*)(header_amsdos->filename));
...
//////////////////////////////////////////////////////////////
int lee_cpc_sna(const char *filename, ts_sbb_header * head_sbb,
		ts_sbb_block * blocks_sbb)
{
	int x;
	...
	if (!x) {
		...
		memcpy(ga_crtc_regs_codec + 0x31 + 18, &head_cpc_sna->GA_inkval,
		       16);
		ga_crtc_regs_codec[0x31 + 18 + 16] =
		    head_cpc_sna->GA_mul_conf_reg;
		ga_crtc_regs_codec[0x31 + 18 + 17] = head_cpc_sna->GA_inkadd;
		copy_data(&blocks_sbb[0], (unsigned char*)ga_crtc_regs_codec);

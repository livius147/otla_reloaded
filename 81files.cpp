...
//////////////////////////////////////////////////////////////
int lee_p(const char *filename, ts_sbb_header * head_sbb,
	  ts_sbb_block * blocks_sbb)
{
	...
	if (!x) {
		...
		if (tamanyo <= 0x400)
			head_sbb->model = 1;
		else if (tamanyo <= 0x800)
			head_sbb->model = 2;
		else if (tamanyo <= 0x4000)
			head_sbb->model = 3;
		else if (tamanyo <= 0xc000)
			head_sbb->model = 4;
		else if (tamanyo <= 0x10000)
			head_sbb->model = 5;
	}
...
int lee_z81(const char *filename, ts_sbb_header * head_sbb,
	    ts_sbb_block * blocks_sbb)
{
	st_snap_81 snap;
	int x;
	...
	if (!x) {
		...
		*p_memory++ = 0;
		...
		snap.SP = leeregistro(p_cpu, (char*)"SP");
		snap.PC = leeregistro(p_cpu, (char*)"PC");

		snap.HL = leeregistro(p_cpu, (char*)"HL ");
		snap.DE = leeregistro(p_cpu, (char*)"DE ");
		snap.BC = leeregistro(p_cpu, (char*)"BC ");
		snap.AF = leeregistro(p_cpu, (char*)"AF ");
		snap.HLx = leeregistro(p_cpu, (char*)"HL_");
		snap.DEx = leeregistro(p_cpu, (char*)"DE_");
		snap.BCx = leeregistro(p_cpu, (char*)"BC_");
		snap.AFx = leeregistro(p_cpu, (char*)"AF_");
		snap.IX = leeregistro(p_cpu, (char*)"IX");
		snap.IY = leeregistro(p_cpu, (char*)"IY");
		snap.R = leeregistro(p_cpu, (char*)"IR") & 0xff;
		snap.I = leeregistro(p_cpu, (char*)"IR") >> 8;
		snap.IM = leeregistro(p_cpu, (char*)"IM");

		snap.EI_DI = leeregistro(p_cpu, (char*)"IF2") & 4;

		snap.NMI = leeregistro(p_zx81, (char*)"NMI");
		snap.HSYNC = leeregistro(p_zx81, (char*)"YNC");
		...
		memcpy(blocks_sbb[1].data, memoria81 + 0x4000,
		       1 + input2 - 0x4000);

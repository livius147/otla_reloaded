...
void make_wave_tables()
{
	...
	wave_table((char*)c_pilot_zx);
	wave_table((char*)c_sync_zx);
	wave_table((char*)c_zero_zx);
	wave_table((char*)c_one_zx);

	wave_table((char*)c_zero_cpc);
	wave_table((char*)c_one_cpc);

	wave_table((char*)c_zero_msx);
	c_one_msx[0] = 10;
	wave_table((char*)c_one_msx);
	c_one_msx[0] = 20;
	memcpy(c_one_msx + 11, c_one_msx + 1, 10);

	c_zero_81[0] = 0.5 + _ACELERA_81_ * (8369) * freq_muestreo / 3500000.0;
	c_one_81[0] =
	    0.5 + _ACELERA_81_ * (5250 + 8369) * freq_muestreo / 3500000.0;
	...
	wave_table((char*)c_pilot_velozwav);
	...
	wave_table((char*)c_sync_velozwav);
	/*
	...
	wave_table((char*)c_sync2_velozwav);

	forma_onda = fa;

	wave_table((char*)c_00_velozwav);
	wave_table((char*)c_01_velozwav);
	wave_table((char*)c_10_velozwav);
	wave_table((char*)c_11_velozwav);

	wave_table((char*)c_t1_velozwav);
	wave_table((char*)c_t2_velozwav);
	wave_table((char*)c_t3_velozwav);
	wave_table((char*)c_t4_velozwav);
}
...
int wav2tzx(const char *file_in, const char *file_out, const char *mens)
{
	...
	if (fwav != NULL) {
		...
		if (ftzx != NULL) {
			...
			memcpy(tzx_file.mensaje.men, mens,
			       tzx_file.mensaje.largo_men);

			tzx_file.direct_record.ID = 0x15;
			tzx_file.direct_record.tstates_sample =
			    0.5 + 3500000.0 / (freq_muestreo);
			...
			if (fwrite(&tzx_file, 1, escritos_tzx, ftzx) !=
			    escritos_tzx)
				return -2;	//res=E_ESCRIBIR;
			fclose(ftzx);
		} else
...
double dura()
{
	return (headerWav.dataChunkSize / headerWav.BlockAlign) /
	    ((double)(freq_muestreo));
}
...
int open_wav(const char *file_out)
{
	...
	if (fwav != NULL) {
		...
		headerWav.BlockAlign =
		    headerWav.NumChannels * (headerWav.BitsPerSample / 8);
		headerWav.ByteRate =
		    headerWav.SampleRate * headerWav.BlockAlign;

		if (fwrite((char *)&headerWav, 1, sizeof(headerWav), fwav) !=
		    sizeof(headerWav))
			res = E_ESCRIBIR;
		headerWav.dataChunkSize = 0;
...
//////////////////////////////////////////////////////
//   Bloque de cabecera/ datos a Wav seg\FAn la norma ZX
//////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////
//   Bloque de cabecera/ datos ?? a Wav seg\FAn la norma CPC
//////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////
//   Bloque de datos a Wav seg\FAn la norma MSX
...
int block2msx_bin_wav(const char *name, const unsigned char *data, int length,
		      unsigned char filetype = 0xd0)
{
	int i, pulsos;
	unsigned char b;
	int res = 0;
